...
describe CreateOmniauthRegistrationExtend do
  describe "call" do
    ...
    let(:command) { Decidim::CreateOmniauthRegistration.new(form, verified_email) }
    ...
    describe "when the form oauth_signature cannot ve verified" do
      ...
      it "raises a InvalidOauthSignature exception" do
        expect { command.call }.to raise_error Decidim::InvalidOauthSignature
      end
    end
    ...
    context "when the form is valid" do
      ...
      describe "user linking" do
        context "with a verified email" do
          ...
          it "links a previously existing user" do
            user = create(:user, email: email, organization: organization)
            expect { command.call }.to change(Decidim::User, :count).by(0)
      ...
      it "creates a new identity" do
        expect do
          command.call
        end.to change(Decidim::Identity, :count).by(1)
        last_identity = Decidim::Identity.last
      ...
      it "confirms the user if the email is already verified" do
        ...
        expect_any_instance_of(Decidim::User).to receive(:skip_confirmation!)
